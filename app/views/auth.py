...
def signup(credentials):
    ...
    return make_response(f"User {credentials['email']} successfully signed up", 201)
...
def login(credentials):
    ...
    if not verify_password(
        password=credentials["password"], password_hash=existing_user.password
    ):
    ...
    return make_response(f"User {credentials['email']} logged in", 200)
...
def logout():
    ...
    logout_user()
    return make_response(f"User {email} logged out", 200)
...
def change_password(passwords):
    ...
    criterias = check_password(
        passwords["new_password"], current_app.config["SECURITY_PASSWORD_LENGTH_MIN"]
    )
    if not criterias["password_ok"]:
        abort(400, json.dumps(criterias))

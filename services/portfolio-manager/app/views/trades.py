...
def validate_trade_params(params):
    ...
    if params["unit_price"] <= 0:
        abort(400, f"Trade unit price must be a positive number")

    if params["quantity"] <= 0:
        abort(400, f"Trade quantity must be a positive number")

    if params["brokerage_fee"] < 0:
        abort(400, f"Trade brokerage fee cannot be a negative number")
